...
class OperationRunner:
    ...
    The first implementation intentionally dispatches only the existing
    ScanRunner. PIVOT/MANUAL are graph/evidence concepts and are rejected by
    execute() until a separately reviewed provider is implemented. This
    keeps the new AttackOperation model from silently becoming a generic
    ...
    def execute(self, operation: AttackOperation, action_id: str) -> AttackOperation:
        action = _action(operation, action_id)
        ...
        if action.kind != ActionKind.SCAN:
            raise OperationError(
                f"action kind '{action.kind.value}' is not executable yet; "
                "it can be represented and approved, but no state-changing "
                "or pivot provider is enabled by this operation layer"
            )
