...
_PROMPT_TEMPLATE = (
    ...
    "(no raw tool output). Write connective narrative prose in the tester's "
    "voice describing how the engagement unfolded from one run to the next "
    "-- what was tried, what came out of it, and why the next step followed. "
    ...
    "ruled out. Respond with plain prose only -- no markdown headers, no "
    "JSON, no code fences.\n\n"
    "Steps, in order:\n{steps}"
)
...
def generate_walkthrough(
    ...
    narrative prose covering them. Read-only: no RunRecord is modified or
    re-saved, unlike core/analysis.py::run_analysis (which overwrites a
    single run's findings) -- a walkthrough is a new, separate artifact."""
    ...
    try:
        narrative = adapter.analyze(prompt)
